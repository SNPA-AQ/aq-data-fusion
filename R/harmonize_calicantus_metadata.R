...
if(readline(prompt = "Please locate your 'Renviron' file with environmental variables definitions (Enter to proceed)")=="") readRenviron(file.choose())

source(paste0(Sys.getenv("R_SCRIPTS_DIR"),'metadata_util.R'))
...
nuts <- readShapePoly(paste0(Sys.getenv("GEOGRAPHIC_DATA_DIR"),
                             "NUTS2016_WGS84_LatLong/NUTS_RG_01M_2016_4326_LEVL_3"))
...
active_stations <- function(Source, Days, Pollutant) {
  ...
  for(i in 1:length(Days)) {
    ff <- paste0(Sys.getenv("CALICANTUS_DATA_DIR"),"obs-data/",
  ...
  if(!is.null(Dat)) Dat <- Dat %>% 
    ...
    select(Name, Lat, Lon)  %>% 
    distinct()
  flog.info(paste0(Pollutant,", ",Source,": found ",nrow(Dat)," active stations in days ",paste(Days,collapse=", ")))
  Dat
}
...
prepare_metadata <- function(Pollutant, Days, all_eea_metadata) {
  ...
  bind_rows(list(
    read_csv_quietly(paste0(Sys.getenv("CALICANTUS_DATA_DIR"),
                            "sites-info/metadata.ARPA-VdA.csv")) %>%
    ...
    read_csv_quietly(paste0(Sys.getenv("CALICANTUS_DATA_DIR"),
                            "sites-info/metadata.ARPA-Lombardia.csv")) %>%
    ...
    read_csv_quietly(paste0(Sys.getenv("CALICANTUS_DATA_DIR"),
                            "sites-info/metadata.APPA-Bolzano.csv")) %>%
      filter(!is.na(LAT)) %>%
    ...
    read_csv_quietly(paste0(Sys.getenv("CALICANTUS_DATA_DIR"),
                            "sites-info/metadata.APPA-Trento.csv")) %>%
    ...
    read_csv_quietly(paste0(Sys.getenv("CALICANTUS_DATA_DIR"),
                            "sites-info/metadata.ARPA-Lazio.csv")) %>%
    ...
    read_csv_quietly(paste0(Sys.getenv("CALICANTUS_DATA_DIR"),
                            "sites-info/metadata.ARPA-Piemonte.csv")) %>%
    ...
    read_csv_quietly(paste0(Sys.getenv("CALICANTUS_DATA_DIR"),
                            "sites-info/metadata.ARPA-Puglia.csv")) %>%
      filter(!is.na(LAT)) %>%
...
to_base <- function(x, base) {
  ...
  if(x%%1!=0 || x<0 || base<2 || base>length(ch) || base%%1!=0) flog.error(paste0("to_base(",x,",",base,")"))
  out <- NULL
  while(x>0) {
...
from_base <- function(x, base) {
  ch <- c(0:9,LETTERS,letters)
  if(x%%1!=0 || x<0 || base<2 || base>length(ch) || base%%1!=0) flog.error(paste0("from_base(",x,",",base,")"))
...
for (poll in c("PM10", "PM2.5", "NO2", "O3")) {
  flog.info(paste0("Processing ",poll))
